...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] flex">
      {/* Sidebar — desktop */}
      <aside className={`hidden md:flex flex-col ${isAdminRoute ? "w-40" : "w-56"} bg-[#1E3A5F] border-r border-gray-100 shrink-0`}>
      ...
      <main className="flex-1 min-w-0 md:overflow-auto">
        ...
        <div className={`${isAdminRoute ? "px-1 sm:px-1 lg:px-2" : "px-4 sm:px-6 lg:px-8"} py-4`}>
          {children || <Outlet />}
        </div>
      </main>
    </div>
